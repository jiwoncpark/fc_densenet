    ...
    with tf.variable_scope(scope):
        for i in xrange(num_layers):
            tmp = batch_activ_conv(input_tensor = current, 
                                   ...
                                   keep_prob = keep_prob, 
                                   scope = 'layer_'+str(i))
            current = tf.concat([current, tmp], axis=len(input_tensor.shape)-1 )
...
def transition_up(shortcut, to_concat, is_training, num_outputs, scope):
    ...
    fn_deconv = tf.layers.conv3d_transpose

    net = tf.concat(to_concat, axis=len(shortcut.shape)-1)
    net = fn_deconv(inputs = net,
                    ...
                    name = scope,
                    use_bias = False)
    net = tf.concat([net, shortcut], axis=len(net.shape)-1)
